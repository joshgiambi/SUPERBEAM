...
async function fixMissingSlices() {
  console.log('Checking for missing CT slices...');
  ...
  const existingImages = await sql`
    SELECT i.sop_instance_uid, i.slice_location, i.instance_number
    FROM images i
  ...
  const existingUIDs = new Set(existingImages.map(img => img.sop_instance_uid));
  const existingSliceLocations = new Set(existingImages.map(img => parseFloat(img.slice_location)));
  ...
  for (const filename of ctFiles) {
    ...
    try {
      ...
      if (!existingUIDs.has(sopInstanceUID)) {
        missingCount++;
        missingImages.push({
          filename,
          sopInstanceUID,
  ...
  if (missingCount > 0) {
    ...
    // Check for gaps
    const allSliceLocations = [...existingSliceLocations, ...missingImages.map(img => img.sliceLocation)].sort((a, b) => a - b);
    
    let gaps = [];
    for (let i = 1; i < allSliceLocations.length; i++) {
      const gap = allSliceLocations[i] - allSliceLocations[i-1];
      if (gap > 3) { // More than 3mm gap
        gaps.push({
          from: allSliceLocations[i-1],
          to: allSliceLocations[i],
          gap: gap
        });
      }
    }
    ...
    if (gaps.length > 0) {
      console.log('\nGaps found:');
      gaps.forEach(g => console.log(`  ${g.from}mm to ${g.to}mm (${g.gap}mm gap)`));
    }
    
    // Now insert the missing images
    console.log('\nInserting missing images...');
    
    for (const img of missingImages) {
      const filepath = path.join(DATASET_PATH, img.filename);
      const buffer = fs.readFileSync(filepath);
      const byteArray = new Uint8Array(buffer);
      const dataSet = dicomParser.parseDicom(byteArray);
